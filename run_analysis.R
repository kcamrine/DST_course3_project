...
# Merges the training and the test sets to create one data set including adding 
# names of columns and identifiers of samples
...
expids <- c(read.table("UCI HAR Dataset/test/subject_test.txt")[,1],
            read.table("UCI HAR Dataset/train/subject_train.txt")[,1]) #subject number
activ <- c(read.table("UCI HAR Dataset/test/y_test.txt")[,1],
           read.table("UCI HAR Dataset/train/y_train.txt")[,1]) #activity number
activity.names <- read.table("UCI HAR Dataset/activity_labels.txt")[,2]

# Appropriately labels the data set with descriptive variable names. 
colnames(total_x) <- c(as.character(read.table("UCI HAR Dataset/features.txt")[,2]))
...
total_x_subset_by_activity_subject <- aggregate( . ~ subject+activity, 
                                                data=total_x_subset, FUN=mean)

#for the long version
#final_summary <- melt(total_x_subset_by_activity_subject,id.vars = c(1,2))

#write output to a table
write.table(total_x_subset_by_activity_subject,file="run_analysis_output_tidy.txt",
            row.name=FALSE)
